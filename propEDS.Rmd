...
Unfortunately, the predictions are useless in the condition that would be most easily used in a diagnostic setting (aligned active). This is still done without training/test data splits, and still ignores co-predictors, which could reduce and increase power respectively. Nevertheless, at first glance, it seems that using hand localization precision is not a usefull _clinical_ predictor of EDS.

## Alternative: Predict group from Beighton scores and localization variance

Here is an alternative approach to this. The regression is not super strong, and it could be that being in the EDS group causes both higher Beighton scores as well as higher localization variance. It would make sense that we can predict group (control vs. EDS) from Beighton scores, as Beighton scores are actually used to diagnose people as having EDS (i.e. using Beighton scores to predict group is circular). However if they share the same source, two things would be true: the two would have high collinearity, and adding localization variance to a classifier to predict group based on Beighton scores would not be better than only predicting group based on Beighton score.
